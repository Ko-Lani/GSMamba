...
class RecurrentMixPrecisionRTModel(VideoRecurrentModel):
    ...
    def optimize_parameters(self, scaler, current_iter):
        ...
        with torch.cuda.amp.autocast(enabled=False):
            ...
            # 🔻 여기에서 spynet grad 수동 초기화
            if self.fix_flow_iter and current_iter < self.fix_flow_iter:
                for name, param in self.net_g.named_parameters():
                    if 'spynet' in name or 'deform' in name:
                        if param.grad is not None:
                            param.grad.zero_()

            # optional gradient-norm clipping (opt-in via train.grad_clip_norm; there was
            # previously no safeguard here at all, so a single large-gradient batch could
            # apply a corrupting update and permanently NaN the weights with no recovery)
            grad_clip_norm = self.opt['train'].get('grad_clip_norm', None)
            if grad_clip_norm:
                scaler.unscale_(self.optimizer_g)
                grad_norm = torch.nn.utils.clip_grad_norm_(self.net_g.parameters(), grad_clip_norm)
                if not torch.isfinite(grad_norm):
                    logger = get_root_logger()
                    logger.warning(f'Non-finite grad norm ({grad_norm}) at iter {current_iter}; skipping this step.')
