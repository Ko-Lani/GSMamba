...
from archs.gsmamba_arch import GSMamba as net
...
def main():
    ...
    parser.add_argument('--task', type=str, default='GSMamba_REDS4', help='task name, used as the results/ subfolder')
    ...
    parser.add_argument('--tile', type=int, nargs='+', default=[100,144,144],
                        help='Tile size, [0,0,0] for no tile during testing (testing as a whole)')
    parser.add_argument('--tile_overlap', type=int, nargs='+', default=[2,50,50],
    ...
    parser.add_argument('--model_path', type=str, default='pretrained/gsmamba_reds_x4.pth', help='model path')
...
def prepare_model_dataset(args):
    ...
    model = net(upscale=4, prop_depth=4, attn_depth=4, pa_frame=2, embed_dims=192, num_heads=8, use_checkpoint_attn=False, use_checkpoint_ffn=False)
    args.scale = 4
    # args.window_size = 8
