...
class ImplicitWarpModule(nn.Module):
    ...
    def forward(self, y, x, flow):
        ...
        if self.use_checkpoint_attn:
            vw, x, wrp = checkpoint(self._forward_impl, y, x, flow)
        else:
            vw, x, wrp = self._forward_impl(y, x, flow)
    ...
    def forward(self, y, x, flow):
        ...
        if self.use_checkpoint_attn:
            x, wrp, grid_wrp_off = checkpoint(self._forward_impl, y, x, flow)
        else:
            x, wrp, grid_wrp_off = self._forward_impl(y, x, flow)
    ...
    def _forward_impl(self, y, x, flow):
    ...
    # def forward(self, y, x, flow):
    #     if self.use_checkpoint_attn:
    #         return checkpoint(self._forward_impl, y, x, flow)
    #     else:
    #         return self._forward_impl(y, x, flow)
    ...
    # def _attention_block(self, qx, kw, vw):
    #     attn = (qx * self.scale) @ kw.transpose(-2, -1)
    #     attn = self.softmax(attn)
    #     out = (attn @ vw).transpose(1, 2).reshape(qx.shape[0], 1, self.dim)
    #     return out
    ...
    #     if self.use_checkpoint_attn:
    #         out = checkpoint(self._attention_block, qx, kw, vw)
    #     else:
    #         out = self._attention_block(qx, kw, vw)
